...
export class ApontamentosTempoService {
  ...
  async stopById(id: number, dto: StopApontamentoDto): Promise<ApontamentosTempo> {
    ...
    if (!apont.dt_hr_inicio) {
      throw new BadRequestException('Registro sem dt_hr_inicio.');
    }

    const dtInicio = new Date(apont.dt_hr_inicio);
    const dtFim = new Date(dto.dt_hr_fim);
    if (dtFim < dtInicio) {
      throw new BadRequestException('dt_hr_fim não pode ser anterior a dt_hr_inicio.');
    }
    ...
    if (typeof dto.quantidade === 'number') {
      apont.quantidade = dto.quantidade;
    }
